...
//
//  $File: ExplorerSim.cs $ $Revision: 1 $
//
// TT Jun-2007:
...
namespace Microsoft.Robotics.Services.ExplorerSim
{
    ...
    class ExplorerSim : DsspServiceBase
    {
        #region constants
...
//        const int MaximumForwardVelocity = 1000; // mm/sec
        ...
        private const string InitialStateUri = ServicePaths.MountPoint + @"/Apps/QUT/Config/ExplorerSim.Config.xml";
        ...
        [ServicePort("/explorersim")]
        ExplorerSimOperations _mainPort = new ExplorerSimOperations();
        #endregion
        ...
        // TT Jun-2007
        // Changed to UseExisting because the slow startup time of the
        // simulator was resulting in a new LRF service being created!
//        [Partner("Laser", Contract = sicklrf.Contract.Identifier, CreationPolicy = PartnerCreationPolicy.UseExistingOrCreate)]
        ...
        public ExplorerSim(DsspServiceCreationPort create)
            : base(create)
        {
        ...
        IEnumerator<ITask> DoDrop(DsspDefaultDrop drop)
        {
            ...
            if (_state.IsActive)
            {
                LogInfo("ExplorerSim service is being dropped while moving, Requesting Stop.");
